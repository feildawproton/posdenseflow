...
class Viewer:
    ...
    def __init__(self, csv_path: str, min_field_res: int, time_res: str, decay_rate: float):
        ...
        n=3
        self.pos_field  = ti.Vector.field(n=3,dtype=ti.f32, shape=(self.lon_pix, self.lat_pix))
        self.__zero_2dscaler(self.pos_field)
        print("shape of pos_field", self.pos_field.shape, "with vector component length", n)
    ...
    def __add_samples2pos(self, samples: np.ndarray):
        ...
        for sample in samples:
            ...
            curr_val = self.pos_field[x,y][2]
            self.pos_field[x,y][0] = 0.0
            self.pos_field[x,y][1] = 0.0
            self.pos_field[x,y][2] = ti.max(1.0, curr_val)
    ...
    def __blur_pos(self,):
        for i, j in self.pos_field:
            ...
            if i > 0 and j > 0 and i < (self.lon_pix - 1) and j < (self.lat_pix - 1):
                z_10 = self.pos_field[i  , j-1][2] - self.pos_field[i, j][2]
                z_01 = self.pos_field[i-1, j  ][2] - self.pos_field[i, j][2]
                z_11 = self.pos_field[i  , j  ][2] - self.pos_field[i, j][2]
                z_21 = self.pos_field[i+1, j  ][2] - self.pos_field[i, j][2]
                z_12 = self.pos_field[i  , j+1][2] - self.pos_field[i, j][2]
                ...
                self.pos_field[i,j][2] = ti.min(val, 1.0)
    ...
    def __zero_edges(self):
        for i, j in self.pos_field:
            if i == 0 or i == (self.pos_field.shape[0] - 1) or j == 0 or j == (self.pos_field.shape[1] - 1):
                self.pos_field[i,j][0] = 0.0
                self.pos_field[i,j][1] = 0.0
                self.pos_field[i,j][2] = 0.0
    ...
    def __increment(self):
        if self.t_n <= self.feeder.max_t:
            ...
            self.my_display.set_3d(self.pos_field)
