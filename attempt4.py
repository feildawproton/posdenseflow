...
class Viewer:
    ...
    def __init__(self, csv_path: str, min_field_res: int, time_res: str, decay_rate: float):
        ...
        self.last_pos   = ti.field(dtype=ti.f32, shape=(self.lon_pix, self.lat_pix))
        self.__zero_2dscaler(self.last_pos)
        print("shape of las pos field", self.last_pos.shape, "lon and lat have flipped for easier viewing!!!")
        
        self.pos_diff   = ti.field(dtype=ti.f32, shape=(self.lon_pix, self.lat_pix))
        self.__zero_2dscaler(self.pos_diff)
        print("shape of pos_diff field", self.pos_diff.shape, "lon and lat have flipped for easier viewing!!!")
    ...
    def __add_samples2pos(self, samples: np.ndarray):
        ...
        for sample in samples:
            ...
            curr_val = self.pos_field[x,y]
            self.pos_field[x,y] = ti.max(1.0, curr_val)
    ...
    def __blur_pos(self,):
        for i, j in self.pos_field:
            ...
            if i > 0 and j > 0 and i < (self.lon_pix - 1) and j < (self.lat_pix - 1):
                ...
                b_11 = self.pos_field[i  , j  ]
                b_21 = self.pos_field[i+1, j  ] 
                b_12 = self.pos_field[i  , j+1] 
                blur = 0.2 * (       b_10 +
                              b_01 + b_11 + b_21 + 
                                     b_12)
                val = blur * self.decay_rate
                self.pos_field[i,j] = ti.min(val, 1.0)
                
    @ti.kernel
    def __pos_difference(self):
        for i, j in self.pos_diff:
            self.pos_diff[i,j] = self.pos_field[i,j]*ti.abs(self.pos_field[i,j] - self.last_pos[i,j])
            
    @ti.kernel
    def __copy_posfield(self):
        for i, j in self.last_pos:
            self.last_pos[i,j] = self.pos_field[i,j]
    ...
    def __increment(self):
        if self.t_n <= self.feeder.max_t:
            ...
            self.__blur_pos()
            self.__pos_difference()
            
            #self.my_display.set_1d(self.pos_field)
            self.my_display.set_1d(self.pos_diff)
